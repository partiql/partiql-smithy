...
This document defines the V1 interface for PartiQL's https://en.wikipedia.org/wiki/Query_plan[Plan] representation. PartiQL's logical plans can be serialized as Ion; see xref:serialization.adoc[PartiQL Logical Plan Serialization (V1)] for details.
...
Plans are represented by a top-level object we'll call "document".
...
See xref:types.adoc[PartiQL Types] for type semantics.
